...
justone <- mydata %>% filter(id=="FOODS_3_586_TX_2_validation")
...
ggplot(demandwfor, aes(resid5))+
  ...
  labs(x = "5% residuals", y = "Count of Observations at each unit error")
...
write.xlsx(demandwfor, "demandwfor_test.xlsx")
...
Pct_Acc <- c(0.5,0.4,0.3,0.2,0.1,0.05,0.02,0.01,0)
metrics_table <- data.frame(Pct_Acc)
metrics_table$Avg_InvOH <- NA
...
# z <- 27 # this is the ROP column number

# set up loop


z <- 34 # this is the ROP column number
...
ggplot(for_plot, aes(date, back_ord))+
  geom_line()

ggplot(for_plot)+
  geom_line(aes(date, demand))+
  labs(x = "Date", y = "Demand in Units")

ggplot(for_plot)+
  geom_line(aes(date, SSone))+
  ylim(0,250)
...
ggplot(for_plot)+
  geom_line(aes(date, SSfifty), color = 'red')+
  geom_line(aes(date, SStwenty), color = 'blue')+
  geom_line(aes(date, SSperfect), color = 'green')+
  labs(x = "Date", y="Safety Stock Levels in Units")

ggplot(for_plot)+
  geom_line(aes(date, ROPfifty), color = 'red')+
  geom_line(aes(date, ROPtwenty), color = 'blue')+
  geom_line(aes(date, ROPperfect), color = 'green')+
  labs(x = "Date", y="ROP Levels in Units")

ggplot(for_plot)+
  geom_line(aes(date, ROQ), color = 'red')+
  ylim(0,400)+
  labs(x= "Date", y = "Reorder Quantity In Units")
