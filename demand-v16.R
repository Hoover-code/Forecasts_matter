...
justone <- mydata %>% filter(id=="FOODS_3_318_WI_3_validation")
...
write.xlsx(justone_t, "singledemand.xlsx")
...
# plot the time series plot for the demand for the single item
autoplot(demand1, demand)+
...
# plot the time series by day of the week 
demand1 %>% gg_season(demand, period = "week")+
  labs(x="Day of Week", y = "Demand")

# plot the time series by month 
demand1 %>% gg_season(demand, period = "month")+
  labs(x="Month", y = "Demand")

# plot the time series by year 
demand1 %>% gg_season(demand, period = "year")+
  labs(x="Year", y = "Demand")

# plot the distribution of the demand in a bar plot

ggplot(demand1, aes(demand))+
  geom_bar()+
  scale_x_continuous(breaks = c(0:20))+
  labs(x="Demand Quantity", y="Frequency of Occurence")

# classical decomposition from fpp3
demand1 %>% model(
...
# ACF analysis
demand1 %>% ACF(demand, lag_max = 28)

# correlegram
demand1 %>%
  ACF(demand, lag_max = 28) %>%
  autoplot()

# add a lag 1 to the tsibble
# demand1$lag1 <- lag(demand1$demand, n=1)
...
# visualize the weekly demand over time
autoplot(demand_wk)
autoplot(demand_mon)
autoplot(demand_yr)
...
ggplot(demandwfor, aes(resid5))+
  geom_bar(color='white')

# save the results in an excel file
write.xlsx(demandwfor, "demandwfor.xlsx")
...
################
# logic check
mean(demandwfor$resid40)
mean(demandwfor$demand)
mean(demandwfor$forty)
...
write.xlsx(demand_mon_recalc, "demand_mon_recalc.xlsx")
...
SS_mon <- SS_mon %>%
  mutate(SSfifty = round(R * sqrt(W) * fiftyMAD),
         SSforty = round(R * sqrt(W) * fortyMAD),
         SSthirty = round(R * sqrt(W) * thirtyMAD),
         SStwenty = round(R * sqrt(W) * twentyMAD),
         SSten = round(R * sqrt(W) * tenMAD),
         SSfive = round(R * sqrt(W) * fiveMAD),
         SStwo = round(R * sqrt(W) * twoMAD),
         SSone = round(R * sqrt(W) * oneMAD),
         SSperfect = round(R * sqrt(W) * perfectMAD))
...
SS_mon$ROPtwo <- NA
SS_mon$ROPone <- NA
SS_mon$ROPperfect <-NA
SS_mon$ROPfifty <- SS_mon$SSfifty + SS_mon$DoLT
SS_mon$ROPforty <- SS_mon$SSforty + SS_mon$DoLT
SS_mon$ROPthirty <- SS_mon$SSthirty + SS_mon$DoLT
SS_mon$ROPtwenty <- SS_mon$SStwenty + SS_mon$DoLT
SS_mon$ROPten <- SS_mon$SSten + SS_mon$DoLT
SS_mon$ROPfive <- SS_mon$SSfive + SS_mon$DoLT
SS_mon$ROPtwo <- SS_mon$SStwo + SS_mon$DoLT
SS_mon$ROPone <- SS_mon$SSone + SS_mon$DoLT
SS_mon$ROPperfect <- SS_mon$SSperfect + SS_mon$DoLT
...
ip_calc <- ip_calc[, c(1:10,34,11:33)]
ip_calc$ROQ <- ip_calc$EOQ
ip_calc_bu <- ip_calc
# write.xlsx(ip_calc, "ip_calc.xlsx")


z <- 32 # this is the ROP column number
ip_calc <- ip_calc_bu
...
# write.xlsx(ip_calc, "after_calcs40.xlsx")
...
for(i in 2:length(ip_calc$date)){
  ...
  ip_calc$ip[i] <- ip_calc$end_inv[i] - ip_calc$back_ord[i] + ip_calc$onOrder[i-1] - ip_calc$deliveries[i]
  
  # make the reorder decision and update on order
...
# write.xlsx(ip_calc, "after_calcs.xlsx")

# https://www.r-bloggers.com/2021/04/ggplot-with-moving-averages/
for_plot <- ip_calc[c(30:1913),]
ggplot(for_plot, aes(date, end_inv))+
  geom_line(color = 'lightgray') +
  geom_ma(ma_fun = SMA, n = 30, color = 'red')+
  ylim(0, 1000) + 
  labs(x="Date", y="Ending Inventory")

################################################################
# create the metrics calculations for each level of forecast accuracy
for_plot <- ip_calc[c(30:1913),]
for_plot$avg_inv <- ((for_plot$beg_inv + for_plot$end_inv) / 2)
avg_inv_oh <- mean(for_plot$avg_inv)
...
for_plot2 <- for_plot[c(600:630),]
ggplot(for_plot2, aes(date, end_inv))+
  geom_line(color = 'lightgray')+
  geom_point(aes(date, placed_on_order, color = 'red'))
